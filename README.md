...
2. 1つ以上のインスタンスプロパティを持つ
3. すべてのインスタンスプロパティが `readonly` 修飾子を持つ
4. `static` プロパティを持たない（`readonly class` では `static` プロパティを持てないため）
5. すべてのプロパティに型宣言がある（`readonly class` では型宣言が必須のため）
6. 親クラスがある場合、親クラスも `readonly` である
